...
if($padding_exist == 1 && $stride == 2){
    $actual_ifm_width = $ARGV[4] + $pad_value -1;
    $actual_ifm_height = $ARGV[5] + $pad_value -1;
}
elsif($padding_exist == 1 && $stride == 1){
    $actual_ifm_width = $ARGV[4] + $pad_value ;
    $actual_ifm_height = $ARGV[5] + $pad_value ;
}
else{
    $actual_ifm_width = $ARGV[4];
    $actual_ifm_height = $ARGV[5];
}



# Number of units in first conv is constant, 3 units in case of RGB or 1 unit in case of gray scale
if($is_RGB == 1){
    $dummy_level = 3; 
}
else{
...
print $fh <<"DONATE";
    ...
    ADDRESS_SIZE_WM         = $clog2(KERNAL_SIZE*KERNAL_SIZE*NUMBER_OF_FILTERS*(${\(ceil($ARGV[6]/$dummy_level))}) ),    
...
if($padding_exist == 1){
    ...
    begin
        if(reset)
            filters_counter <= 0;
        else if( (filters_counter == (NUMBER_OF_FILTERS-1) & ifm_address_with_padding_tick) )
            filters_counter <= 0;
        else if(ifm_address_with_padding_tick)
            filters_counter <= filters_counter + 1'b1;
    end
    
    assign filters_counter_tick = ( filters_counter == (NUMBER_OF_FILTERS-1) & ifm_address_with_padding_tick );
...
if($padding_exist == 1){
    print $fh <<"DONATE";
        start_internal <= ifm_address_with_padding_tick;
DONATE
}
else{
    print $fh <<"DONATE";
...
if($padding_exist == 1){
	...
	localparam COUNTER_NOT_READY_SIZE = $clog2( (STRIDE-1)*\$rtoi(\$ceil((IFM_WIDTH_PAD/STRIDE)))+(KERNAL_SIZE/STRIDE-1));
DONATE
}
else{
    print $fh <<"DONATE";
    localparam COUNTER_READY_SIZE     = $clog2( (IFM_WIDTH-KERNAL_SIZE)/STRIDE + 1 );
	localparam COUNTER_NOT_READY_SIZE = $clog2( (STRIDE-1)*\$rtoi(\$ceil((IFM_WIDTH/STRIDE)))+(KERNAL_SIZE/STRIDE-1));
DONATE
}
...
print $fh <<"DONATE";
    ...
    assign  counter_fifo_tick = (counter_fifo == \$rtoi(\$ceil( FIFO_SIZE/STRIDE ))-1);
...
if($padding_exist == 1){
    print $fh <<"DONATE";
    assign  counter_not_ready_tick = (counter_not_ready == ( (STRIDE-1)*\$rtoi(\$ceil(IFM_WIDTH_PAD/STRIDE))+\$rtoi(\$ceil(KERNAL_SIZE/STRIDE-1))-1));
DONATE
}
else{
    print $fh <<"DONATE";
    assign  counter_not_ready_tick = (counter_not_ready == ( (STRIDE-1)*\$rtoi(\$ceil(IFM_WIDTH/STRIDE))+\$rtoi(\$ceil(KERNAL_SIZE/STRIDE-1))-1));
DONATE
}
...
/////////////////////////////////////
    ...
    // Address Write Counter to write results in OFM Memory
    always @(posedge clk, posedge reset)
    begin
        if(reset)
...
my $signal_bits = 1;
chdir "./Modules";
system("perl delay.pl $delay_cycles $signal_bits $ARGV[10]");
