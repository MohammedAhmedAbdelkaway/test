...
my $padding_exist = $ARGV[10];
my $stride = $ARGV[9];

# input Width and height with padding value added #
my $pad_value  = ($ARGV[5] - 1) ;

if($padding_exist == 1 && ($stride == 2)){
    $actual_ifm_width = $ARGV[2] + $pad_value -1;
    $actual_ifm_height = $ARGV[3] + $pad_value -1;
}
elsif($padding_exist == 1 && ($stride == 1)){
    $actual_ifm_width = $ARGV[2] + $pad_value ;
    $actual_ifm_height = $ARGV[3] + $pad_value ;
}
else{
    $actual_ifm_width = $ARGV[2];
    $actual_ifm_height = $ARGV[3];
}

if($padding_exist == 1){
    if($actual_ifm_width  == $ARGV[5]){
...
if($padding_exist == 1){
    ...
    begin
        if(reset)
            filters_counter <= 0;
        else if( filters_counter == (($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS)-1) & ifm_address_with_padding_tick )
            filters_counter <= 0;
        else if(ifm_address_with_padding_tick)
            filters_counter <= filters_counter + 1'b1;
    end
    
    assign filters_counter_tick = (filters_counter == (($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS)-1) & ifm_address_with_padding_tick);
...
/////////////////////////////////////
    ...
    begin
        ...
        else if(ifm_address_read_next == IFM_HEIGHT_NEXT * IFM_WIDTH_NEXT-1)      
            ifm_address_read_next <= {ADDRESS_SIZE_NEXT_IFM{1'b0}};      
...
if($padding_exist == 1){
    $signal_bits = ceil(log(($actual_ifm_width - $ARGV[5] + 1)*($actual_ifm_width- $ARGV[5] + 1))/log(2));
}
else{
    $signal_bits = ceil(log(($ARGV[2] - $ARGV[5] + 1)*($ARGV[2] - $ARGV[5] + 1))/log(2));
}
