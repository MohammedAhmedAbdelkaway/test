...
if($ARGV[3]==1){
	...
	$module_name = "max_pooling_S${\($ARGV[0])}"; 
}


=begin
if ($ARGV[3] == 0){
	system("perl max.pl  $ARGV[1] $ARGV[2] $ARGV[1] $ARGV[6] $ARGV[4] $ARGV[7] $ARGV[8] $ARGV[9]");
}
=end
...
for ($i = 1; $i < $levels_number; $i = $i + 1){
	...
	for($j = 1; $j <= $levels[$i] + ($odd_flag % 2);$j = $j + 1){
		if($ARGV[3] == 1){
			...
			print $fh "\twire\t[DATA_WIDTH - 1 : 0]	max_$i$under_Score$j;\n";
		}
	}
	print $fh "\n";
}
...
for ($i = 1; $i < $levels_number; $i = $i + 1){
	...
	for($j = 1; $j <= $levels[$i] + ($odd_flag % 2);$j = $j + 1){
		...
		if($ARGV[3] == 1){
			...
			print $fh "\treg \t[DATA_WIDTH - 1 : 0]	reg_max_$i$under_Score$j;\n";
		}
...
if($ARGV[3] == 1){
	...
	print $fh "\twire [$data_width - 1 : 0]	maxIFM;\n\n";
}

############### Generation of Average pooling ##############
if($ARGV[3]==1){
...
############### Generation of max pooling ##############
	...
	$i = 1;
		...
		for($j = 1; $j <= $levels[$i] ;$j = $j + 1){
			...
			print $fh "\t$max_name  #(.DATA_WIDTH(DATA_WIDTH), .ARITH_TYPE(ARITH_TYPE))\t\tmaxINST_$i$under_Score$j\t(.in1(pool_data_in_$jj), .in2(pool_data_in_${\($jj+1)}), .out(max_$i$under_Score$j));\n";
			$jj = $jj + 2;
		}
		if($odd_flag % 2){
			print $fh "\n\tassign max_$i$under_Score$j = pool_data_in_$jj;\n";
		}
		print $fh "\n";
	...
	for ($i = 2; $i < $levels_number; $i = $i + 1){
		...
		$jj = 1;
		for($j = 1; $j <= $levels[$i] ;$j = $j + 1){
			print $fh "\t$max_name  #(.DATA_WIDTH(DATA_WIDTH), .ARITH_TYPE(ARITH_TYPE))\t\tmaxINST_$i$under_Score$j\t(.in1(reg_max_${\($i-1)}$under_Score$jj), .in2(reg_max_${\($i-1)}$under_Score${\($jj+1)}), .out(max_$i$under_Score$j));\n";
			$jj = $jj + 2;
		}
		if($odd_flag % 2){
			print $fh "\n\tassign max_$i$under_Score$j = reg_max_${\($i-1)}$under_Score$jj;\n";
		}
		print $fh "\n";
	}
...
if($ARGV[3] == 1){
	...
	$odd_flag = $odd_flag + ($levels[$i-1] % 2);
	for($j = 1; $j <= $levels[$i] + ($odd_flag % 2);$j = $j + 1){
		print $fh "\t\t\treg_max_$i$under_Score$j <= 0;\n";
	}
	print $fh "\n";
}
...
if($ARGV[3] == 1){
	...
	$odd_flag = $odd_flag + ($levels[$i-1] % 2);
	for($j = 1; $j <= $levels[$i] + ($odd_flag % 2);$j = $j + 1){
		print $fh "\t\t\treg_max_$i$under_Score$j <= \tmax_$i$under_Score$j ;\n";
	}
	print $fh "\n";
}
...
DONATE
	...
	print $fh <<"DONATE";
	assign maxIFM = reg_max_${\($i-1)}$under_Score${\($jj-2)};
DONATE
}
