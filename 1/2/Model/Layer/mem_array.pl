...
$module_name = "mem_array_U${\($units)}_D${\($IFM_number)}_H$ARGV[4]_W$ARGV[5]";
#U for units and D for IFM Depth H IFM_HIEGHT W IFM_WIDTH
...
my $addressbits = ceil(log($ARGV[4]*$ARGV[5]) / log(2));
chdir "./CU_DP/Modules/operations";

system("perl Demux1toN.pl $divisionOutput $ARGV[7]");

system("perl Demux1toN_Mbit.pl $divisionOutput $addressbits $ARGV[7]");
...
system("perl mem_unit.pl $units $ARGV[7]"); 
...
mem_unit_$units #( .DATA_WIDTH(DATA_WIDTH),.IFM_HEIGHT(IFM_HEIGHT), .IFM_WIDTH(IFM_WIDTH))
M$i(  .clk(clk),
DONATE
...
for($k = 1;$k <= $ifm_init_number; $k = $k + 1){
		...
		print $fh <<"DONATE";
	$dual_port_name #(.DATA_WIDTH(DATA_WIDTH), .MEM_SIZE(IFM_HEIGHT*IFM_WIDTH)) 
	IFM$accu_var (
    .clk(clk),
